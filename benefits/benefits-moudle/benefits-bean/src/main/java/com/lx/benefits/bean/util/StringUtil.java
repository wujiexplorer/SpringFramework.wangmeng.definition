...
public class StringUtil extends org.apache.commons.lang3.StringUtils
{
    ...
    /**
     * 把字符串按照separator分割后解析成Long list
     * @param inputStr inputStr
     * @param separator separator
     * @return List
     */
    public static List<Long> getLongIdList(String inputStr, String separator) {
        if (null == inputStr || inputStr.isEmpty()) {
            return null;
        }
        List<Long> longIdList = new ArrayList<>();
        separator = null == separator || separator.isEmpty() ? "," : separator;
        String[] longIdArr = inputStr.split(separator);
        for (String longIdStr: longIdArr) {
            if (null == longIdStr || longIdStr.isEmpty()) {
                continue;
            }
            Long longId = Long.parseLong(longIdStr);
            if (longIdList.contains(longId)) {
                continue;
            }
